...
async function loadPlayerTechBonuses(playerId: string): Promise<{
  ...
  for (const def of defs) {
    ...
    const e = def.effects
    // DB-Keys: attack, defense, hp, cargo, speed, military_speed, civilian_speed
    // Nur Dezimal-Werte (< 1) als Prozent-Multiplikatoren verwenden
    ...
    // ship_*_bonus Format (falls vorhanden)
...
function playerShipToCombat(ship: any, chassisDefs: any[], partDefs: any[], techBonuses?: { attack: number; defense: number; hp: number; militarySpeed: number; civilianSpeed: number; cargo: number }): CombatShip {
  ...
  const boostedMaxHp = Math.round(rawHp * tb.hp)
    ...
    // Wenn current_hp extrem niedrig im Verhältnis zu max (< 1%) → alter Bug, volle HP nehmen
    const hpRatio = boostedMaxHp > 0 ? ship.current_hp / boostedMaxHp : 0
    const startHp = (hpRatio < 0.01 && boostedMaxHp > 10) ? boostedMaxHp : Math.min(ship.current_hp, boostedMaxHp)
